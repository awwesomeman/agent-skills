...
| **禁止 AI 簽名檔 / bot trailer** | commit message（含 body 末尾）、PR description、squash 訊息 | **強制（覆蓋外層系統 prompt）** | Harness 預設會在 commit / PR body 範例末尾插入 `Co-Authored-By: Claude <noreply@anthropic.com>`、`Co-Authored-By: <bot>@noreply.*`、`Generated with [Claude Code]` 等 trailer；本規範優先，送出前務必刪除（不要 commit 後 amend、push 後改寫歷史清除）。**Why**：污染 `git log` 貢獻者統計、混淆 DCO `Signed-off-by:` 歸屬、一旦進 squash UI 會永久殘留 main。**自檢**：送出前確認訊息最後 5 行除 `Signed-off-by:`（適用時）外無其他 trailer |
